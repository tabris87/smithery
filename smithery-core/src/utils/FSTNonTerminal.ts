...
export class FSTNonTerminal extends FSTNode {
  ...
  public deepClone(overrideProperties: OverrideProperties): FSTNode {
    const clone = new FSTNonTerminal(
      overrideProperties?.type || this.getType(),
      overrideProperties?.name || this.getName(),
      this.getChildren().map((c) => c.deepClone())
    );
    clone.setRawContent(JSON.parse(JSON.stringify(this.getRawContent())));
    return clone;
  }
  ...
  public shallowClone(): FSTNode {
    const clone = new FSTNonTerminal(this.getType(), this.getName(), []);
    clone.setRawContent(JSON.parse(JSON.stringify(this.getRawContent())));
    return clone;
  }
}
