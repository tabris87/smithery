...
export abstract class FSTNode {
  ...
  private _nodeRawContent: { [key: string]: any };
  ...
  public compatibleWith(node: FSTNode): boolean {
    return (
      (this._name + '').localeCompare(node.getName() + '') === 0 &&
      this._type.localeCompare(node.getType()) === 0
    );
  }
  //#endregion
  ...
  public setRawContent(content: { [key: string]: any }): void {
    this._nodeRawContent = content;
  }

  public getRawContent(): { [key: string]: any } {
    return this._nodeRawContent;
  }
   ...
   */
  public abstract deepClone(overrideProperties?: OverrideProperties): FSTNode;
