...
exports.rule = {
    apply: (baseFST, featureFST, context) => {
        ...
        const baseAST = context.parserFactory.getParser(baseFST.ending).parse(baseFST.content, {});
        const featureAST = context.parserFactory.getParser(featureFST.ending).parse(featureFST.content, {});
        baseAST.featureName = baseFST.featureName;
        featureAST.featureName = featureFST.featureName;
        // 2. impose both structures
        const resultingAST = context.impose(baseAST, featureAST, context.parserFactory.getParser(baseFST.ending).getVisitorKeys());
        // 3. recreate the node for further processing
        const returningNode = featureFST.clone();
        returningNode.content = context.generatorFactory.getGenerator(featureFST.ending).generate(resultingAST);
