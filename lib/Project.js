        ...
        if (oModule) {
            ...
            if (oModule.rules) {
                if (!Array.isArray(oModule.rules)) {
                    console.error('The rules provided by the plugin ' + oPlugin.name + 'are not as an array, therefore they can not be loaded');
                } else {
    ...
    setConfig(sConfigName) {
        if (typeof sConfigName === 'object') {
            if (typeof sConfigName.config !== 'undefined') {
                ...
                var oConfigObject = this._configs.filter(config => config.name === sConfigName.name);
                if (oConfigObject.length === 0) {
                    throw new Error('Config for ' + sConfigName.name + ' does not exist withing configs');
                }

                this._config = oConfigObject[0];
            }
        } else {
            if (this._configs === null) {
                throw new Error('No configs given');
            ...
            var oConfigAny = this._configs.filter(config => config.name === sConfigName);
            if (oConfigAny.length === 0) {
                throw new Error('Config for ' + sConfigName + ' does not exist withing configs');
            }

            this._config = oConfigAny[0];
        }
    }
