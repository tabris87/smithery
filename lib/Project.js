...
class Project {
    constructor(oOptions) {
        ...
        oOptions = oOptions || {}; //just to prevent missing config options access
        ...
        try {
            ...
            if (configPath.endsWith('package.json') && !config.featureCLI) {
                throw new Error('featureCLI not a package.json root level found!');
            } else {
            ...
            //we can continue
            if (!(oOptions.configs && oOptions.buildFolder && oOptions.projectFiles)) {
                const errorText = "Failed to read configuration for project " +
                    `at "${this._workingDir}". Error: ${err.message}`;
        ...
        if (fs.existsSync(path.join(this._workingDir, config.configs))) {
            ...
            this._configs = oOptions.configs;
        }

        //setting up the src and destination for the build
        this._buildTarget = oOptions.buildFolder || config.buildFolder;
        ...
        this._plugins = oOptions.plugins || config.plugins;
        ...
        this._projectAST = this._parser.parse(path.join(this._workingDir, oOptions.projectFiles || config.projectFiles), 'DIR');
    }
