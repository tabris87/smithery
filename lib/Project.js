...
class Project {
    ...
    constructor(mOptions) {
        ...
        try {
            ...
            if (configPath.endsWith('package.json') && !config.featureCLI) {
                throw new Error('featureCLI not at package.json root level found!');
            } else {
        ...
        if (config.configs && typeof config.configs === "string" && fs.existsSync(path.join(this._workingDir, config.configs))) {
            ...
            this._configs = mOptions.configs;
        }
    ...
    _loadPlugin(oPlugin) {
        ...
        var oModule = this._loadLocally(oPlugin);
        if (!oModule) {
            debugger;
            /* console.log(path.join(process.argv[1].replace('index.js'), '../../../', oPlugin.name));
            var bExists = fs.existsSync(path.join(process.argv[1].replace('index.js'), '../../../', oPlugin.name));
    ...
    _loadLocally(oPlugin) {
        try {
            return require(oPlugin.name);
        } catch (err) {
            return;
        }
    }

    _loadGlobally(oPlugin) {
        return;
    }
