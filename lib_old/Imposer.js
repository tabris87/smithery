            ...
            if (typeof aBaseChilds === 'object' && Array.isArray(aBaseChilds)) {
                aBaseChilds.forEach((oChildB) => {
                    ...
                    if (oChildF.length === 1) {
                        ...
                        var oSubImposeChildOne = this.impose(oChildB, oChildF, mVisitorKeys);
                        /* aResultingChilds.push(this.impose(oChildB, oChildF, mVisitorKeys)); */
                        if (Array.isArray(oSubImposeChildOne)) {
                            aResultingChilds = aResultingChilds.concat(oSubImposeChildOne);
                        } else {
                            aResultingChilds.push(oSubImposeChildOne);
                        }
                    ...
                    if (oChildF.length > 1) {
                        ...
                        var oSubImposeChildMany = this.impose(oChildB, oChildF, mVisitorKeys);
                        /* aResultingChilds.push(this.impose(oChildB, oChildF, mVisitorKeys)); */
                        if (Array.isArray(oSubImposeChildMany)) {
                            aResultingChilds = aResultingChilds.concat(oSubImposeChildMany);
                        } else {
                            aResultingChilds.push(oSubImposeChildMany);
                        }
